...
def postprocess_qa_predictions(
    ...
    for example_id, prediction in all_predictions.items():
        if len(prediction) > max_answer_length:
            logger.warning(f"Prediction for example {example_id} exceeds max_answer_length")
        elif len(prediction) < 2 and prediction != "":
            logger.warning(f"Very short non-empty prediction for example {example_id}")
