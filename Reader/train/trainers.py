...
# Add Custom- trainers
class QuestionAnsweringHSTrainer(Trainer):
    # 기존 코드는 그대로 유지
    def __init__(self, *args, eval_examples=None, post_process_function=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.eval_examples = eval_examples
    ...
    def hyperparameter_search(self, n_trials=10, n_jobs=-1):
        def objective(trial):
            # 하이퍼파라미터 공간 정의
            params = {
            ...
            return metrics["f1"]  # F1 점수를 최적화 목표로 설정
        ...
        optimizer.fit(self.train_dataset)
        
        # 최적의 하이퍼파라미터 및 성능 출력
        print("Best hyperparameters:", optimizer.best_params_)
        print("Best F1 score:", optimizer.best_score_)
        
        # 최적의 하이퍼파라미터로 모델 업데이트
        self.args = TrainingArguments(
            output_dir=self.args.output_dir,
